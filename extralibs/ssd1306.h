...
#if !defined(SSD1306_64X32) && !defined(SSD1306_72X40) && !defined(SSD1306_128X32) && !defined(SSD1306_128X64) &&      \
	!defined(SH1107_128x128) && !(defined(SSD1306_W) && defined(SSD1306_H) && defined(SSD1306_OFFSET))
 ...
 */
uint8_t ssd1306_data(const uint8_t* data, int sz) { return ssd1306_pkt_send(data, sz, 0); }
...
const uint8_t ssd1306_init_array[] = {
	...
	0xf0, // Set precharge properties.  THIS IS A LIE  This has todo with timing.  <<< This makes it go brrrrrrrrr
 ...
 */
void ssd1306_setbuf(uint8_t color) { memset(ssd1306_buffer, color ? 0xFF : 0x00, sizeof(ssd1306_buffer)); }
 ...
 */
void ssd1306_drawImage(uint32_t x, uint32_t y, const unsigned char* input, uint32_t width, uint32_t height,
		       uint32_t color_mode) {
	uint32_t x_absolute;
	uint32_t y_absolute;
	uint32_t pixel;
	...
	for (uint32_t line = 0; line < height; line++) {
		...
		// SSD1306 is in vertical mode, yet we want to draw horizontally, which necessitates assembling the
		// output bytes from the input data bitmask for current pixel in vertical (output) byte
		...
		for (uint32_t byte = 0; byte < bytes_to_draw; byte++) {
			...
			for (pixel = 0; pixel < 8; pixel++) {
				...
				// looking at the horizontal display, we're drawing bytes bottom to top, not left to
				// right, hence y / 8
				...
				switch (color_mode) {
					case 0:
						...
						ssd1306_buffer[buffer_addr] = (ssd1306_buffer[buffer_addr] & ~v_mask) |
									      (input_pixel ? v_mask : 0);
						break;
					case 1:
 ...
 */
int gfx_abs(int x) { return (x < 0) ? -x : x; }
